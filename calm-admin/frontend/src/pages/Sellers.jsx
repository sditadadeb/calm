    ...
    return (
      <div className="flex items-center justify-center h-[60vh]">
        <div className="text-center">
          ...
          <p className={`mt-4 ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>{t('sellers.loading')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className={`rounded-2xl p-6 border border-l-4 border-l-[#0081FF] ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center justify-between">
            ...
            <div className="w-12 h-12 bg-[#0081FF]/20 rounded-xl flex items-center justify-center">
              <Users className="w-6 h-6 text-[#0081FF]" />
            </div>
          </div>
        </div>
        ...
        <div className={`rounded-2xl p-6 border border-l-4 border-l-amber-500 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center justify-between">
            ...
            <div className="w-12 h-12 bg-[#0056b3]/20 rounded-xl flex items-center justify-center">
      ...
      {sellers.length === 0 ? (
        <div className={`rounded-2xl p-12 text-center border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
          ...
          <h3 className={`text-lg font-semibold mb-2 ${isDark ? 'text-white' : 'text-gray-800'}`}>{t('sellers.noSellers')}</h3>
          <p className={isDark ? 'text-slate-400' : 'text-gray-500'}>{t('sellers.syncData')}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sellers.map((seller, index) => (
            <div 
              key={seller.userId}
              className={`rounded-2xl p-6 border transition-all relative overflow-hidden ${isDark ? 'bg-slate-800 border-slate-700 hover:border-[#0081FF]/50' : 'bg-white border-gray-200 hover:border-[#0081FF]/50 hover:shadow-lg'}`}
            >
              {/* Rank Badge */}
              {index < 3 && (
                <div className={`absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center ${
                  index === 0 ? 'bg-gradient-to-br from-[#0081FF] to-[#0862C5]' :
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${isDark ? 'bg-gradient-to-br from-slate-700 to-slate-600' : 'bg-gradient-to-br from-gray-100 to-gray-200'}`}>
                  <span className="text-xl font-bold text-[#0081FF]">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                ...
                <div className={`rounded-xl p-4 text-center ${isDark ? 'bg-slate-700/50' : 'bg-gray-50'}`}>
                  ...
                  <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>{t('sellers.conversion')}</p>
                </div>
              </div>
              ...
              <div className="space-y-3 mb-6">
                <div>
                  <div className="flex items-center justify-between text-sm mb-1">
                    <span className={isDark ? 'text-slate-400' : 'text-gray-500'}>{t('sellers.sales')}</span>
                ...
                <div>
                  <div className="flex items-center justify-between text-sm mb-1">
                    <span className={isDark ? 'text-slate-400' : 'text-gray-500'}>{t('sellers.noSale')}</span>
              ...
              <div className={`flex items-center justify-between pt-4 border-t ${isDark ? 'border-slate-700' : 'border-gray-200'}`}>
                ...
                >
                  <Eye className="w-3 h-3" /> {t('sellers.viewAttendances')}
                </Link>
              </div>
            </div>
          ))}
