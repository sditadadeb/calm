...
export default function Recommendations() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!loading && isAdmin && (
        <div className={`rounded-xl p-4 border flex items-center justify-end gap-2 ${isDark ? 'bg-ink-raised border-line' : 'bg-white border-gray-200'}`}>
          <button
      ...
      {analyzing && analysisProgress.total > 0 && (
        <div className={`rounded-xl border p-4 ${isDark ? 'bg-ink-raised border-line' : 'bg-white border-gray-200'}`}>
      ...
      <div className={`rounded-2xl border overflow-hidden ${isDark ? 'bg-ink-raised border-line' : 'bg-white border-gray-200'}`}>
        <div className={`flex items-center justify-between p-4 border-b ${isDark ? 'border-line' : 'border-gray-200'}`}>
          <button 
            onClick={prevSection}
        ...
        {/* Progress indicator */}
        <div className={`flex gap-1 px-4 py-2 ${isDark ? 'bg-white/[0.03]' : 'bg-gray-50'}`}>
          {sections.map((section) => (
            <div
              key={section.id}
      ...
      {/* Content Area */}
      <div className={`rounded-2xl border p-6 ${isDark ? 'bg-ink-raised border-line' : 'bg-white border-gray-200'}`}>
        {renderSection()}
      </div>
