...
const sections = [
  ...
  { id: 5, name: 'Perfil Agente', icon: User, color: '#8b5cf6' },
...
const THRESHOLDS = {
  ...
  conversion: { min: 40, label: 'Resolución mínima esperada (%)' },
  sellerScore: { min: 6, label: 'Score mínimo del agente' },
...
export default function Recommendations() {
  ...
  // Generar datos basados en agentes reales del dashboard
  ...
  const conversationFlowData = hasAdvancedData && vendorMetrics.length > 0
    ? [
        ...vendorMetrics.slice(0, 5).map(v => {
          ...
          return {
            name: v.userName?.split(' ')[0] || 'Agente',
            ...normalized
          };
        }),
        (() => {
    ...
    : sellers.length > 0 
      ? [
          ...sellers.slice(0, 5).map((s, idx) => ({
            name: s.userName?.split(' ')[0] || `Agente ${idx + 1}`,
  ...
  const objectionTypes = hasAdvancedData ? [
    ...
    { type: 'No respondidas', count: advancedMetrics?.totalObjections?.unanswered || 0, example: 'Ciudadano plantea duda, agente no responde', color: '#6b7280' },
  ...
  // Seleccionar agente para perfil - usar datos avanzados si están disponibles
  ...
  const aiInsights = [
    { type: 'warning', text: 'La resolución baja cuando el ciudadano plantea dudas y el agente no responde.' },
    { type: 'success', text: 'Los agentes que hacen más preguntas de clarificación tienen mejor tasa de resolución.' },
    { type: 'info', text: 'El minuto 7-9 suele ser clave: muchas consultas no resueltas se definen en ese rango.' },
    { type: 'warning', text: 'Sucursal Centro tiene 25% menos escucha activa que el promedio.' },
    { type: 'success', text: 'Las atenciones resueltas duran en promedio más que las no resueltas.' },
  ];
  ...
  const comparisonData = {
    conVenta: { 
      ...
      ratioAgente: hasAdvancedData ? Math.round(avgVendorTalk - 10) : Math.round(45 + avgConversion * 0.2), 
    ...
    sinVenta: { 
      ...
      ratioAgente: hasAdvancedData ? Math.round(avgVendorTalk + 15) : Math.round(65 + (100 - avgConversion) * 0.1), 
  ...
  // Función para generar recomendación basada en datos avanzados del agente
  const getVendorRecommendation = (vendorName, seller) => {
    ...
    if (hasAdvancedData && vendorData) {
      ...
      if (conv < 35) {
        ...
        if (topLoss) {
          return { 
            recommendation: `Resolución ${Math.round(conv)}%. Frase de abandono más común: "${topLoss.phrase}" (min ~${Math.round(avgMinute)}). Entrenar respuesta a esta objeción`, 
            priority: 'media' 
          };
        }
        return { 
          recommendation: `Resolución ${Math.round(conv)}% (${totalSales}/${totalTranscriptions}). Enfocarse en técnicas de retención en minutos 6-10 de la conversación`, 
          priority: 'media' 
        };
      }
      ...
      return { recommendation: `Buen desempeño general. Resolución ${Math.round(conv)}% - seguir mejorando`, priority: 'baja' };
    }
    
    // Fallback con datos básicos
    if (conv < 25) {
      return { recommendation: `Solo ${Math.round(conv)}% de resolución (${totalSales}/${totalTranscriptions}). Requiere acompañamiento urgente`, priority: 'alta' };
    }
    if (conv < 40) {
      return { recommendation: `Resolución ${Math.round(conv)}% - debajo del objetivo. Reforzar cierre de atenciones`, priority: 'media' };
    }
    return { recommendation: `Resolución ${Math.round(conv)}%. Mantener nivel`, priority: 'baja' };
  };
  ...
  const getBranchRecommendation = (branchName, branch, index) => {
    ...
    if (conversion < 25) {
      ...
      if (topLoss) {
        return { 
          recommendation: `Solo ${Math.round(conversion)}% de resolución. Principal causa de no resolución: "${topLoss.phrase}". Capacitar al equipo en esta objeción`, 
          priority: 'alta' 
        };
      }
      return { 
        recommendation: `Solo ${Math.round(conversion)}% de resolución (${totalSales}/${totalTranscriptions}). Capacitar en manejo de consultas y derivación`, 
        priority: 'alta' 
      };
    }
    ...
    if (conversion < 40) {
      ...
      if (avgMinute > 0) {
        return { 
          recommendation: `Resolución ${Math.round(conversion)}%. Momento crítico: minuto ${Math.round(avgMinute)}. Reforzar técnica de cierre en ese punto`, 
          priority: 'alta' 
        };
      }
      return { 
        recommendation: `Resolución ${Math.round(conversion)}% - debajo del objetivo 40%. Reforzar seguimiento y cierre de consultas`, 
        priority: 'alta' 
      };
    }
    ...
    if (conversion >= 50) {
      return { 
        recommendation: `Excelente resolución (${Math.round(conversion)}%). Documentar mejores prácticas para replicar en otras sucursales`, 
        priority: 'baja' 
      };
    }
    
    return { 
      recommendation: `Resolución ${Math.round(conversion)}%, Score ${avgScore.toFixed(1)}. Mantener estándares`, 
      priority: 'baja' 
    };
  };
  ...
  const actionableRecommendations = {
    vendors: sellers.length > 0 
      ? sellers.slice(0, 4).map(s => {
          ...
          return { name: s.userName || 'Agente', recommendation, priority };
        })
    ...
    switch (activeSection) {
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-slate-700/50 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  <h4 className="font-semibold text-gray-700">Por Agente (Resuelto vs No resuelto)</h4>
                  <InfoTooltip text={`Compara el confidence score promedio de cada agente:\n\n🟢 Resuelto: Score en atenciones resueltas\n🔴 No resuelto: Score en atenciones no resueltas\n\nLa diferencia indica cuánto impacta la confianza en la resolución.`}>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={confidenceScoreData.byVendor}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="conVenta" name="Resuelto" fill="#22c55e" />
                    <Bar dataKey="sinVenta" name="No resuelto" fill="#ef4444" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ...
            <div className="bg-gradient-to-r from-green-50 to-red-50 rounded-xl p-4 border">
              <div className="flex justify-between items-center">
                <div className="text-center flex-1">
                  ...
                  <p className="text-xs text-slate-500">Score promedio en resueltos</p>
                </div>
                ...
                <div className="text-center flex-1">
                  ...
                  <p className="text-xs text-slate-500">Score promedio no resueltos</p>
                </div>
              </div>
            </div>
          </div>
        ...
        return (
          <div className="space-y-6">
            <div>
              ...
              <p className="text-sm text-slate-500 mb-4">
                Patrones comunes en atenciones no resueltas.
              </p>
            </div>
      ...
      case 5:
        return (
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-bold text-white mb-2">Perfil del Agente</h3>
              <p className="text-sm text-slate-500 mb-4">
                Análisis multidimensional de la atención.
                {hasAdvancedData 
              ...
              <div className="bg-slate-700/50 rounded-xl p-4">
                ...
                <ResponsiveContainer width="100%" height={280}>
                  <RadarChart data={radarData}>
                    ...
                    <Radar name="Agente" dataKey="A" stroke="#8b5cf6" fill="#8b5cf6" fillOpacity={0.5} />
                  </RadarChart>
                </ResponsiveContainer>
              </div>
      ...
      case 7:
        return (
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-bold text-white mb-2">Comparador: Resuelto vs No resuelto</h3>
              <p className="text-sm text-slate-500 mb-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-green-50 border border-green-200 rounded-xl p-5">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-400">% Tiempo agente</span>
                    <span className="font-bold text-green-700">{comparisonData.conVenta.ratioAgente}%</span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div className="bg-red-50 border border-red-200 rounded-xl p-5">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-400">% Tiempo agente</span>
                    <span className="font-bold text-red-700">{comparisonData.sinVenta.ratioAgente}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Objeciones promedio</span>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
              <p className="text-sm text-blue-800">
                ...
                y el agente habla {comparisonData.sinVenta.ratioAgente - comparisonData.conVenta.ratioAgente}% menos, 
                dejando más espacio para el ciudadano.
              </p>
            </div>
          </div>
        );
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <h4 className="font-semibold text-gray-700 flex items-center gap-2">
                <User className="w-4 h-4" /> Por Agente
              </h4>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className={`rounded-xl border p-4 text-center ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
          ...
          <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>Agentes activos</p>
        </div>
      </div>
    </div>
  );
